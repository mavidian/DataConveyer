//EtlOrchestrator_tests_RecordFilterTransformer.cs
//
...
{
   public class EtlOrchestrator_tests_RecordFilterTransformer
   {
      ...
      private IEnumerable<string> _intakeLines()
      {
         ...
         yield return "@pRECTYPE=XYZ,@pNAME=Susan,@pNUM=1023";
         yield return "EOF";
      }
      ...
      public EtlOrchestrator_tests_RecordFilterTransformer()
      {
      ...
      [Fact]
      public void ProcessPipeline_AcceptAllRecords_SameDataAsInput()
      {
         //arrange
         _config.RecordFilterPredicate = rec => true;  // accept every record
         ...
         kvRec["RECTYPE"].Should().Be("XYZ");
         kvRec["NAME"].Should().Be("Susan");
         kvRec["NUM"].Should().Be(1023);

         kvRec = resultingClusters[2][1];
      ...
      [Fact]
      public void ProcessPipeline_RejectAllRecords_EmptyOutput()
      {
         //arrange
         _config.RecordFilterPredicate = rec => false;  // reject every record
      ...
      [Fact]
      public void ProcessPipeline_RejectSusan_ProperRecordsRejected()
      {
         //arrange
         _config.RecordFilterPredicate = rec => RejectSusan(rec);  // reject if NAME=Susan
         ...
         resultingClusters.Should().HaveCount(3);  //all 3 cluster are included

         resultingClusters[0].Count.Should().Be(3);  //1st cluster lost 1 record
         resultingClusters[0][0]["NUM"].Should().Be(123);
         resultingClusters[0][1]["NUM"].Should().Be(223);
         resultingClusters[0][2]["NUM"].Should().Be(423);

         resultingClusters[1].Count.Should().Be(5);  //2nd cluster didn't lose any records
         resultingClusters[1][0]["NUM"].Should().Be(523);
         resultingClusters[1][1]["NUM"].Should().Be(623);
         resultingClusters[1][2]["NUM"].Should().Be(723);
         resultingClusters[1][3]["NUM"].Should().Be(823);
         resultingClusters[1][3]["NAME"].Should().Be("Jane");
         resultingClusters[1][4]["NUM"].Should().Be(923);

         resultingClusters[2].Count.Should().Be(1);  //3rd cluster lost 1 record
         resultingClusters[2][0]["NUM"].Should().BeNull();  //NUM does not exist (only EOF)
         resultingClusters[2][0].GetItem("EOF").ItemDef.Type.Should().Be(ItemType.String);
      }


      [Fact]
      public void ProcessPipeline_RejectSusanAndMary_FirstClusterRejectedAndMore()
      {
         //arrange
         _config.RecordFilterPredicate = rec => RejectSusanAndMary(rec);  // reject if NAME=Susan or Mary
         ...
         resultingClusters.Should().HaveCount(2);  //1st cluster disappeared as it lost all records
         resultingClusters[0].Count.Should().Be(3);  //copy of clstr 2, which lost 2 records 

         resultingClusters[0][0]["NUM"].Should().Be(723);
         resultingClusters[0][1]["NUM"].Should().Be(823);
         resultingClusters[0][1]["NAME"].Should().Be("Jane");
         resultingClusters[0][2]["NUM"].Should().Be(923);

         resultingClusters[1].Count.Should().Be(1);  //3rd cluster lost 1 record
         resultingClusters[1][0]["NUM"].Should().BeNull();  //NUM does not exist (only EOF)
         resultingClusters[1][0].GetItem("EOF").ItemDef.Type.Should().Be(ItemType.String);
      }


      /// <summary>
      /// Helper function to determine if NAME value in the record is Susan
      /// </summary>
      /// <param name="record"></param>
      /// <returns>true if NAME is Susan; false otherwise</returns>
      private bool RejectSusan(IRecord record)
      {
         return (string)record["NAME"] != "Susan";
      }

      /// <summary>
      /// Helper function to determine if NAME value in the record is Susan or Mary
      /// </summary>
      /// <param name="record"></param>
      /// <returns>true if NAME neither Susan nor Mary; false otherwise</returns>
      private bool RejectSusanAndMary(IRecord record)
      {
         return (string)record["NAME"] != "Susan" && (string)record["NAME"] != "Mary";
      }
   }
}
